...
- **`run_tests.sh`** — 35 assertions against `bw_export.sh` using a stateful mock
...
Expect `PASS=35 FAIL=0`. As a control, the pre-fix script fails 16 of them:

```bash
git show 57aa29d:bw_export.sh > /tmp/bw_export.orig.sh
SCRIPT=/tmp/bw_export.orig.sh bash tests/run_tests.sh   # PASS=19 FAIL=16
```
